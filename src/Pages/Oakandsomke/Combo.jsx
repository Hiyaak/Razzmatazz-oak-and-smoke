...
const toggleSubProduct = (id) => {
  ...
  const getComboQuantity = (comboId) => {
    const cartItem = cart.find(
      (item) => item.cartItemId === `combo-${comboId}`,
    );
      ...
      <div className="w-full md:w-[42%] h-screen border-r border-gray-200 flex flex-col">
        ...
        <div className="flex-1 overflow-y-auto px-4 pb-4 [&::-webkit-scrollbar]:hidden [-ms-overflow-style:none] [scrollbar-width:none]">
          <div className="grid grid-cols-2 gap-4 mt-6 pb-4">
            {combos.map((combo) => {
              const images = combo.subproductIds || [];
              ...
              return (
                ...
                >
                  {/* Image Collage */}
                  {/* <div className='w-full h-56 mb-2 overflow-hidden rounded-sm grid grid-cols-2 grid-rows-2 gap-[2px] bg-gray-100'>
                ...
                `}
                  ...
                  {/* <div className="w-full h-56 mb-2 rounded-sm overflow-hidden">
                    <div
                      ...
                      style={{
                        ...
                        gridAutoRows: "1fr", // makes images shrink to fit same height
                      }}
                    >
                      {images
                  ...
                  <div className="w-full h-56 mb-2 rounded-sm overflow-hidden flex gap-[3px]">
  {images
    .filter((item) => item?.image)
    .map((item) => (
      <div key={item._id} className="flex-1 overflow-hidden">
        <img
          src={`${ImagePath}${item.image}`}
          alt={item.name}
          className="w-full h-full object-cover"
